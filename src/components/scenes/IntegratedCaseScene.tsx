...
export function IntegratedCaseScene({ title, steps }: IntegratedCaseSceneProps) {
  ...
  const inView = useInView(sceneRef, { margin: "220px 0px 220px 0px" });
  const reducedMotion = useReducedMotion();
  const shouldLoop = inView && !reducedMotion;

  return (
    <div className="integrated-case" ref={sceneRef} aria-label={`Historia visual: ${title}`}>
      ...
      <div className="integrated-case__track" style={{ "--case-steps": steps.length } as React.CSSProperties}>
        ...
        <motion.i
          ...
          transition={{ duration: LOOP, repeat: shouldLoop ? Infinity : 0, ease: "linear", times: [0, .06, .94, 1] }}
        />
        ...
        {steps.map((stage, index) => {
          ...
          return (
            <motion.article
              ...
              key={stage.title}
              animate={{ opacity: [.28, .28, 1, 1, .32], y: [5, 5, 0, 0, -2] }}
              transition={{ duration: LOOP, repeat: shouldLoop ? Infinity : 0, times: [0, reveal, Math.min(reveal + .06, .9), Math.min(reveal + .22, .95), 1] }}
            >
